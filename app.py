    ...
    if active_tab == TAB_LABELS[9]:
        ...
        if not df_route.empty:
            ...
            if len(stations_to_visit) > 1:
                ...
                fig_rt = px.line_map(
                    route_df, lat=lat_col, lon=lon_col, hover_name='Unvan', zoom=9, map_style="open-street-map"
                )
                fig_rt.add_trace(go.Scattermap(
                    lat=route_df[lat_col], lon=route_df[lon_col], mode='markers+text',
                    marker=go.scattermap.Marker(size=14, color='green'),
