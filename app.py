...
# ==========================================
# 🎨 PROFESYONEL CSS & TASARIM AYARLARI
# (Senin eski CSS'lerin ile yeni Glassmorphism birleştirildi)
# ==========================================
st.markdown("""
<style>
    ...
    .main-text { font-size: 2.5em; font-weight: 900; color: #00ff41; text-shadow: 0 0 10px #00ff41; text-transform: uppercase; margin-bottom: 20px; }
    .sub-text { font-size: 1.2em; color: #ffffff; opacity: 0.8; }
    .blink { animation: blinker 1s linear infinite; }
    @keyframes blinker { 50% { opacity: 0; } }
    .data-flash { font-size: 3em; font-weight: bold; color: #00ff41; text-shadow: 0 0 20px #00ff41; animation: popIn 0.2s ease-out; }
    @keyframes popIn { 0% { transform: scale(0.5); opacity: 0; } 100% { transform: scale(1); opacity: 1; } }

    /* --- KPI KUTULARI (GLASSMORPHISM) --- */
    ...
    .stTabs [data-baseweb="tab"]:hover { color: #3498db; background-color: #eef6fc; }
    .stTabs [aria-selected="true"] { background-color: #3498db !important; color: white !important; }

    /* --- ÖZEL KUTULAR (Senin Kodun) --- */
    ...
    .insight-box-danger { padding: 15px; border-radius: 8px; background-color: #ffeaea; border-left: 5px solid #dc3545; color: #721c24; margin-bottom: 15px; box-shadow: 0 2px 5px rgba(0,0,0,0.05); }
    ...
    .robo-card:hover { transform: translateY(-5px); box-shadow: 0 8px 16px rgba(0,0,0,0.15); }
    ...
    .duyuru-kutusu {
        ...
        margin-bottom: 25px; border: 2px solid white;
        animation: glowing 1.5s infinite alternate;
    }

    /* SEKME RENKLERİ */
    button[data-testid="stTab"]:nth-child(3) p { color: #28a745 !important; font-weight: 800 !important; animation: blinker-green 1.5s linear infinite; }
    ...
    button[data-testid="stTab"]:nth-child(12) p { color: #ff2b2b !important; font-weight: 800 !important; animation: blinker-red 1.5s linear infinite; }
...
BOLGE_TANIMLARI = {
    ...
    "Marmara": ["İSTANBUL", "BURSA", "BALIKESİR", "SAKARYA", "TEKİRDAĞ", "KOCAELİ", "EDİRNE", "ÇANAKKALE", "KIRKLARELİ", "BİLECİK", "YALOVA"],
    "Ege": ["İZMİR", "ANTALYA", "MANİSA", "AYDIN", "DENİZLİ", "MUĞLA", "KÜTAHYA", "ISPARTA", "BURDUR", "UŞAK"],
    "Karadeniz": ["SAMSUN", "TRABZON", "ORDU", "SİVAS", "TOKAT", "ERZURUM", "ÇORUM", "GİRESUN", "AMASYA", "RİZE", "KARS", "SİNOP", "AĞRI", "ERZİNCAN", "ARTVİN", "BAYBURT", "GÜMÜŞHANE", "IĞDIR", "ARDAHAN", "TUNCELİ"],
    "Güneydoğu": ["ADANA", "GAZİANTEP", "MERSİN", "ŞANLIURFA", "DİYARBAKIR", "HATAY", "KAHRAMANMARAŞ", "MALATYA", "MARDİN", "BATMAN", "ELAZIĞ", "ADIYAMAN", "VAN", "OSMANİYE", "ŞIRNAK", "MUŞ", "BİTLİS", "SİİRT", "KARAMAN", "KİLİS"]
}
...
# ==============================================================================
# 🔥 YENİLENEN TABLO FONKSİYONU: AGGRID (PROFESYONEL)
# ==============================================================================
def show_details_table(dataframe, target_date_col, extra_cols=None):
    ...
    # Tarih formatı (String'e çevir ki AgGrid düzgün göstersin)
    ...
    gb.configure_column("Unvan", pinned='left', width=250, header_name="Bayi Unvanı")
    gb.configure_column("Kalan_Gun", header_name="Kalan Gün", type=["numericColumn", "numberColumnFilter"], width=120)
    
    # Risk Durumu Renklendirme
    risk_cell_style = {
        'styleConditions': [
            {'condition': "params.value == 'KRİTİK'", 'style': {'color': 'red', 'fontWeight': 'bold', 'backgroundColor': '#ffe6e6'}},
            {'condition': "params.value == 'GÜVENLİ'", 'style': {'color': 'green', 'fontWeight': 'bold', 'backgroundColor': '#e6ffe6'}}
        ]
    }
    gb.configure_column("Risk_Durumu", cellStyle=risk_cell_style)
    
    # Excel İndirme Butonu (Eski stil buffer yerine AgGrid'in kendi özelliği veya bu butonu koruyabiliriz. Butonu koruyoruz)
    buffer = io.BytesIO()
    try:
        with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
            dataframe.to_excel(writer, index=False, sheet_name='Bayi Listesi')
        st.download_button("📥 Excel İndir (Tüm Veri)", buffer.getvalue(), "Bayi_Listesi.xlsx", "application/vnd.ms-excel")
    except: pass
...
def main():
    ...
    <div class="duyuru-kutusu">
        🚀 GÜZEL ENERJİ ANALİZ PLATFORMU YENİLENDİ! <br>
        <span style="font-size:0.7em; font-weight:normal;">AgGrid & Yeni Arayüz Yayında.</span>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_info3:
        ...
        • 📊 <a href="https://pazarpayi.streamlit.app/" target="_blank">EPDK LPG AYLIK SEKTÖR RAPORU</a><br>
    ...
    with tabs[2]:
        ...
        if not df_dom.empty:
            ...
            icon_layer = pdk.Layer(
                type="IconLayer", data=leaders, get_icon="icon_data",
                get_position='[lon, lat]', get_size=30, size_scale=1, pickable=True
            )
    ...
    with tabs[9]:
        ...
        if 'İlçe' in df_robo.columns: df_robo['Tam_Konum'] = df_robo['İl'] + " - " + df_robo['İlçe']
        else: df_robo['Tam_Konum'] = df_robo['İl']
        ...
        if not df_robo.empty:
            ...
            if not hero_city_counts.empty:
                ...
                top_hero_city, top_hero_city_cnt, share_in_top_city = "Yok", 0, 0
            ...
            if not hero_dist_counts.empty:
                ...
                top_hero_dist, top_hero_dist_cnt, share_in_top_dist = "Yok", 0, 0
            ...
            with c2:
                ...
                <li><b>Pazarın Kalbi (İlçe):</b> En yoğun rekabet <b>{market_top_districts.index[0]}</b> bölgesinde dönüyor.</li>
            ...
            with c3:
                ...
                if not hero_df.empty:
                    ...
                    for loc in my_locs:
                        ...
                        if share > 0.25: strong_presence.append(f"{loc} (%{int(share*100)})")
                    all_market_locs = df_robo['Tam_Konum'].unique()
                    for loc in all_market_locs:
                        if loc not in my_locs:
                            market_count = market_avgs.get(loc, 0)
                            if market_count > 5: ghost_zones.append(f"{loc} ({market_count} İstasyon)")
                    
                    st.markdown(f"""
                    <div class="robo-card">
                    <ul class="robo-list">
                    <li><b>Dominant Bölgeler (>%25 Pay):</b> <b>{', '.join(strong_presence[:5]) if strong_presence else 'Yok'}</b>.</li>
                    <li><b>Hayalet Bölgeler (Biz Yokuz!):</b> <b>{', '.join(ghost_zones[:5]) if ghost_zones else 'Yok'}</b>.</li>
                    </ul>
                    </div>
                    """, unsafe_allow_html=True)
                else: st.info("Coğrafi analiz için veri yetersiz.")

            with c4:
                st.markdown("#### 📅 4. Gelecek Projeksiyonu")
                if 'Bitis_Yili' in df_robo.columns and future_years_market:
                    ...
                    for y in years_list:
                        ...
                        proj_data.append({"Yıl": int(y), "Toplam Pazar": m_val, "Güzel Enerji": h_val, "Pay (%)": f"%{share_potential:.1f}"})
            ...
            if not big_bosses:
                ...
                for index, row in summary_df.iterrows():
                    ...
                    with st.expander(f"🔻 {vkn} - {unvan} ({count} İstasyon)"):
                        ...
                        show_details_table(sub_df, target_date_col)
        else:
             st.warning("Vergi No sütunu bulunamadı veya veri yok.")
    ...
    with tabs[11]:
        ...
        selected_label = st.selectbox("🔎 Bayi Seçin (Yazmaya başlayın...):", options=[""] + search_options, index=0)
        ...
        if selected_label:
            ...
            for c in df.columns:
                if "ADRES" in c.upper():
                    adres_col = c
                    break
            adres = row.get(adres_col) if adres_col else f"{ilce} / {il}"
            vergi_no = '-'
            for c in df.columns:
